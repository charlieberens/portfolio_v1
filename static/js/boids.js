if(typeof window !== 'undefined' && window.document) {
	...
	const paintColor = 'rgba(255,255,255,.2)';
			
	let boidsRunning = true;
	
	let width = document.getElementById('boids-header').offsetWidth;
	let height = document.getElementById('boids-header').offsetHeight;
	...
	let initField = () => {
		...
		window.addEventListener('resize', () => {
			width = document.getElementById('boids-header').offsetWidth;
			height = document.getElementById('boids-header').offsetHeight;
			canvas.width = width;
			canvas.height = height;
			c.fillStyle = paintColor;
		});
